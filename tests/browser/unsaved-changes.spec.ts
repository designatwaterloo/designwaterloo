...
test('unfinished position remains after cancelled navigation',async({page})=>{
 ...
 await page.getByRole('link',{name:'Home',exact:true}).click();await page.getByRole('dialog').getByRole('button',{name:'Keep editing'}).click();await expect(page.getByLabel('Title',{exact:true})).toHaveValue('Unfinished role');
});
...
test('leadership back warns before discarding unsaved roles',async({page})=>{
 ...
 await page.getByRole('button',{name:'+ Add leadership role',exact:true}).click();await page.getByLabel('Title',{exact:true}).fill('Unfinished lead');
 ...
 await page.getByRole('dialog').getByRole('button',{name:'Keep editing'}).click();
 await expect(page.getByLabel('Title',{exact:true})).toHaveValue('Unfinished lead');
 await page.getByRole('button',{name:'Back to experience',exact:true}).click();
