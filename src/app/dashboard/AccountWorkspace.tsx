...
import { useGuardedNavigation, useUnsavedChanges } from "@/components/UnsavedChanges";
...
import { ImportedPosition, newPosition, positionError, positionFieldError, positionIncomplete, sortPositions } from '@/lib/experience-import';
import { workSequences } from '@/lib/work-sequences';
import { BIO_WORD_LIMIT, bioWordCount } from '@/lib/bio';
...
function AnimatedTitle({children}:{children:string}) {
 ...
 useEffect(()=>{
  if(!ready.current){ready.current=true;if(children!==shown)setShown(children);return;}
  if(children===shown)return;
  setOutgoing(shown);setShown(children);
 },[children,shown]);
 return <h1 className={styles.pageTitle}>{outgoing&&<span className={styles.pageTitleOut} aria-hidden="true" onAnimationEnd={()=>setOutgoing(null)}>{outgoing}</span>}<span className={outgoing?styles.pageTitleIn:undefined}>{shown}</span></h1>;
}
export default function AccountWorkspace({member}:{member:Member}) {
 ...
 const [leadershipPositions,setLeadershipPositions]=useState<ImportedPosition[]>([]);
 const [loadingPositions,setLoadingPositions]=useState(false);
 const [openPosition,setOpenPosition]=useState<string|null>(null);
 const [listEditing,setListEditing]=useState<null|'Experience'|'Leadership'>(null);
 const [requiredFailed,setRequiredFailed]=useState(false);
 const [pasteMode,setPasteMode]=useState(false);
 const savedPositions=useRef('[]');
 useUnsavedChanges(!!editing && (busy || (editing==='Experience'||editing==='Leadership' ? JSON.stringify(positions)!==savedPositions.current : JSON.stringify(draft)!==JSON.stringify(member))));
 ...
 function cancel() {setDraft(member);setEditing(null);setError('');setRequiredFailed(false);setPasteMode(false);if(tab==='Experience')void openExperience(false);}
 function leaveExperienceView() {setError('');setRequiredFailed(false);setOpenPosition(null);setEditing(null);setListEditing(null);setPasteMode(false);void openExperience(false);}
 function backToExperience() {requestNavigation('/dashboard#experience',leaveExperienceView);}
 ...
 function importedPositions(rows:{id?:string;company?:string|null;organization?:string|null;position_title?:string|null;start_month?:string|null;start_year?:string|null;end_month?:string|null;end_year?:string|null;is_current?:boolean;description?:string|null;location?:string|null;employment_type?:string|null;link?:string|null}[]|null) {
  return sortPositions((rows||[]).map(p=>({id:p.id||crypto.randomUUID(),company:'company' in p?p.company||'':p.organization||'',position_title:p.position_title||'',start_month:p.start_month||null,start_year:p.start_year||'',end_month:'end_month' in p?p.end_month||null:null,end_year:'end_year' in p?p.end_year||null:null,is_current:!!p.is_current,description:'description' in p?p.description||'':'',location:'location' in p?p.location||'':'',employment_type:'employment_type' in p?p.employment_type||'':'',link:p.link||null})));
 }
 async function openExperience(edit = true, leadership = false, positionId?: string) {
  setLoadingPositions(true);setError('');if(edit)setNotice('');
  if(!edit){
   setPasteMode(false);
   const [experience,leadershipRows]=await Promise.all([db.from('member_experiences').select('*').eq('member_id',member.id),db.from('member_leadership').select('*').eq('member_id',member.id)]);
   if(experience.error||leadershipRows.error)setError('Couldn’t load your experience. Try again.');
   else {setPositions(importedPositions(experience.data));setLeadershipPositions(importedPositions(leadershipRows.data));}
   setOpenPosition(null);setListEditing(null);setLoadingPositions(false);return;
  }
  if(positionId||leadership)setPasteMode(false);
  ...
  else {const loadedPositions=importedPositions(r.data);setPositions(loadedPositions);savedPositions.current=JSON.stringify(loadedPositions);setOpenPosition(positionId??null);setListEditing(leadership?'Leadership':'Experience');setEditing(leadership?'Leadership':'Experience');}
  setLoadingPositions(false);
 }
 async function addRole(leadership=false) {
  setPasteMode(false);
  if((editing==='Experience'&&!leadership)||(editing==='Leadership'&&leadership)){
   const added=newPosition();setPositions(current=>[added,...current]);setOpenPosition(added.id);return;
  }
  setLoadingPositions(true);setError('');setNotice('');
  const r=await db.from(leadership?'member_leadership':'member_experiences').select('*').eq('member_id',member.id);
  if(r.error){setError('Couldn’t load your experience. Try again.');setLoadingPositions(false);return;}
  const existing=importedPositions(r.data);const added=newPosition();
  setPositions([added,...existing]);savedPositions.current=JSON.stringify(existing);setOpenPosition(added.id);setListEditing(leadership?'Leadership':'Experience');setEditing(leadership?'Leadership':'Experience');
  setLoadingPositions(false);
 }
 async function saveExperience() {
  const invalid=positionError(positions);
  if(invalid){
    const incomplete=positions.find(p=>positionIncomplete(p)||positionFieldError(p));
    if(incomplete){setError('');setRequiredFailed(true);setOpenPosition(incomplete.id);return;}
    setError(invalid);return;
  }
  setRequiredFailed(false);
  ...
  try {
   ...
   await refreshMember();void fetch("/api/revalidate-profile",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({slug:member.slug})}).catch(()=>{});setEditing(null);setPasteMode(false);setNotice('Experience saved.');void openExperience(false);
  }catch(e){setError(e instanceof Error?e.message:'Couldn’t save.');}finally{setBusy(false);}
 }
 ...
 {label:'Positions',page:'Experience',keywords:'jobs work experience linkedin'}, {label:'Leadership',page:'Experience',keywords:'community volunteer'},
 ...
 function jumpTo(item:{label:string;page:Tab}){navigate(item.page,item.label);}
 function navigate(next:Tab, target?:string){
  if(next===tab&&!target)return;
  const url=`/dashboard#${next.toLowerCase()}`;
  requestNavigation(url,()=>{setDraft(member);setEditing(null);setTab(next);window.history.replaceState(window.history.state,'',url);setNotice('');setError('');setSectionQuery('');if(target)setJumpTarget(target);if(next==='Experience')void openExperience(false);});
 }
 ...
 const locked=!!editing||!!listEditing;
 const experienceFocus=editing==='Experience'||listEditing==='Experience';
 const leadershipFocus=editing==='Leadership'||listEditing==='Leadership';
 const focused=experienceFocus||leadershipFocus;
 const pageTitle=leadershipFocus?'Leadership':experienceFocus?'Positions':tab==='Profile'?'Your profile':tab;
 const roleRow=(p:ImportedPosition,leadership=false,showPencil=false)=>{const label=`Edit ${p.position_title||(leadership?'leadership role':'position')}`;const meta=[p.company||(leadership?'Add organization':'Add company'),p.start_year?`${p.start_year}${p.is_current?' – Present':p.end_year?` – ${p.end_year}`:''}`:''].filter(Boolean).join(' · ');return <div key={p.id} className={styles.experienceRow}><div className={styles.experienceIdentity}><p>{p.position_title||(leadership?'New role':'New position')}</p><p className={styles.experienceOrg}>{meta}</p></div>{showPencil&&<button type="button" className={styles.editButton} disabled={loadingPositions} onClick={()=>void openExperience(true,leadership,p.id)} aria-label={label} title={label}><PencilIcon width={20} height={20} aria-hidden="true"/></button>}</div>;};
 ...
 const bioText=String(draft.bio||'');
 const bioCount=bioWordCount(bioText);
 const bioOver=bioCount>BIO_WORD_LIMIT;
 const bioField=<label className={styles.field}><div className={styles.bioEditor} data-over={bioOver||undefined}><textarea aria-label="Bio" name="bio" value={bioText} onChange={e=>change({bio:e.target.value})} aria-describedby="bio-word-count" aria-invalid={bioOver}/><span id="bio-word-count" className={styles.bioCount} data-over={bioOver||undefined} aria-live="polite">{bioCount} / {BIO_WORD_LIMIT}</span></div></label>;
 return <main data-account-workspace className={styles.workspace}>
  ...
  <aside className={styles.sidebar}><div className={styles.identity}><span className={styles.monogram}>{member.profile_image_url?<Image unoptimized src={member.profile_image_url} width={40} height={40} alt=""/>:<>{member.first_name[0]}{member.last_name[0]}</>}</span><div><strong>{member.first_name} {member.last_name}</strong><p>@{member.slug}</p></div></div>
   <div className={styles.publication}>
    <div className={styles.statusGroup}><span className={styles.status}><i data-status={status}/>{label}</span><PublicationHelp/></div>
    {status==='approved'?<button className={styles.secondaryButton} disabled={busy||locked} onClick={()=>setConfirmAction('unpublish')}>Unpublish</button>:(status==='draft'||status==='rejected')&&<button className={styles.secondaryButton} disabled={busy||locked} onClick={()=>setConfirmAction('submit')}>Submit for review</button>}
    {status==='approved'&&member.is_approved&&<Link href={`/@${member.slug}`} className={styles.primaryButton} aria-disabled={locked} tabIndex={locked?-1:undefined} onClick={e=>{if(locked)e.preventDefault();}}>View public profile ↗</Link>}
   </div>
   <div className={styles.sectionSearch}><label><MagnifyingGlassIcon width={16} height={16} aria-hidden="true"/><span className="sr-only">Search sections</span><input type="search" placeholder="Search sections…" value={sectionQuery} disabled={locked} onChange={e=>setSectionQuery(e.target.value)} onKeyDown={e=>{if(e.key==='Escape')setSectionQuery('');if(e.key==='Enter'&&sectionResults[0]){e.preventDefault();jumpTo(sectionResults[0]);}}}/></label>{sectionQuery.trim()&&!locked&&<div className={styles.sectionResults} aria-label="Section search results">{sectionResults.length?sectionResults.map(item=><button key={item.label} onClick={()=>jumpTo(item)}><span>{item.label}</span><small>{item.page}</small></button>):<p role="status">No sections found.</p>}</div>}</div><nav aria-label="Account workspace">{(['Profile','Experience','Availability','Account'] as Tab[]).map(t=><button type="button" key={t} aria-current={tab===t?'page':undefined} disabled={locked&&t!==tab} onClick={()=>navigate(t)}>{(() => {const Icon={Profile:UserCircleIcon,Experience:BriefcaseIcon,Availability:CalendarDaysIcon,Account:Cog6ToothIcon}[t];return <Icon width={18} height={18} aria-hidden="true"/>;})()}{t}</button>)}{canReview(member,user)&&<Link href={member.is_admin?"/admin/members":"/admin"} className={styles.adminLink} aria-disabled={locked} tabIndex={locked?-1:undefined} onClick={e=>{if(locked)e.preventDefault();}}><ShieldCheckIcon width={18} height={18} aria-hidden="true"/>Admin</Link>}</nav><div className={styles.sidebarBottom}><Link href="/directory" className={styles.directoryButton} aria-disabled={locked} tabIndex={locked?-1:undefined} onClick={e=>{if(locked)e.preventDefault();}}><ArrowLeftIcon width={18} height={18} aria-hidden="true"/><span>Back to directory</span></Link><button type="button" className={styles.signOut} disabled={busy||locked} onClick={()=>signOut()}><ArrowRightStartOnRectangleIcon width={18} height={18} aria-hidden="true"/>Sign out</button></div></aside>
  <div className={styles.panel}><RecruitmentBanner dark /><header className={styles.pageHeader} data-focus={focused||undefined}>
    <div className={styles.pageBackSlot} data-visible={focused||undefined}><div className={styles.pageBackSlotInner}><button type="button" className={styles.pageBack} disabled={busy||!focused} tabIndex={focused?undefined:-1} onClick={backToExperience}><ArrowLeftIcon width={18} height={18} aria-hidden="true"/>Back to experience</button></div></div>
    <div className={styles.sectionHeading}>
     <div className={styles.pageTitleBlock}><AnimatedTitle>{pageTitle}</AnimatedTitle><p className={styles.eyebrow} data-hidden={focused||undefined} aria-hidden={focused||undefined}>{{Profile:'Manage your public profile.',Experience:'Your work and community roles.',Availability:'Keep your work terms up to date.',Account:'Manage your account details.'}[tab]}</p></div>
     {focused&&<div className={styles.pageHeaderActions}>{leadershipFocus?<button type="button" className={styles.secondaryButton} disabled={busy||loadingPositions} onClick={()=>void addRole(true)}>+ Add leadership role</button>:<><button type="button" className={styles.secondaryButton} disabled={busy||loadingPositions} onClick={()=>void addRole()}>+ Add position</button>{editing!=='Experience'&&<button type="button" className={styles.secondaryButton} disabled={loadingPositions||!!editing} onClick={()=>{setPasteMode(true);void openExperience();}}>Paste from LinkedIn</button>}</>}</div>}
    </div>
   </header>
   ...
   {tab==='Profile'&&<>
    ...
    {section('Bio','',<p>{member.bio||'Add a short introduction.'}</p>,bioField,()=>void save({bio:draft.bio}))}
   ...
   {tab==='Experience'&&<section className={styles.singleSection}>
    {editing!=='Leadership'&&listEditing!=='Leadership'&&<>{!experienceFocus&&<div className={styles.sectionHeading}><h2>Positions</h2><div className={styles.sectionActions}><button type="button" className={styles.secondaryButton} disabled={loadingPositions||!!editing} onClick={()=>setListEditing('Experience')}>Edit experience</button><button type="button" className={styles.secondaryButton} disabled={loadingPositions||!!editing} onClick={()=>{setPasteMode(true);void openExperience();}}>Paste from LinkedIn</button></div></div>}
    {editing==='Experience'?<ExperienceFields hideAdd hidePaste={!pasteMode} showRequired={requiredFailed} value={positions} onChange={setPositions} disabled={busy} initialOpen={openPosition}/>:positions.length>0?<div className={styles.experienceList}>{positions.map(p=>roleRow(p,false,listEditing==='Experience'))}</div>:<p>{loadingPositions?"Loading your positions…":"No positions added yet."}</p>}</>}
    {editing!=='Experience'&&listEditing!=='Experience'&&<div className={leadershipFocus?undefined:styles.replay}>{!leadershipFocus&&<div className={styles.sectionHeading}><h2>Leadership</h2><div className={styles.sectionActions}><button type="button" className={styles.secondaryButton} disabled={loadingPositions||!!editing} onClick={()=>setListEditing('Leadership')}>Edit leadership</button></div></div>}
    {editing==='Leadership'?<ExperienceFields leadership hideAdd showRequired={requiredFailed} value={positions} onChange={setPositions} disabled={busy} initialOpen={openPosition}/>:leadershipPositions.length>0?<div className={styles.experienceList}>{leadershipPositions.map(p=>roleRow(p,true,listEditing==='Leadership'))}</div>:<p>{loadingPositions?"Loading your leadership…":"No leadership added yet."}</p>}</div>}
    {(editing==='Experience'||editing==='Leadership')&&controls(()=>void saveExperience())}
   </section>}
   ...
   {editing&&<p className={styles.editHint}>Changes aren’t saved yet. Save them here, or choose whether to discard them when you leave.</p>}
  </div>
 </main>;
}
