...
export default function AccountWorkspace({member}:{member:Member}) {
 ...
 async function openExperience(edit = true, leadership = false) {
  ...
  else {const loadedPositions=sortPositions((r.data||[]).map(p=>({...p,company:'company' in p?p.company:p.organization,position_title:p.position_title||'',start_year:p.start_year||'',end_month:'end_month' in p?p.end_month:null,end_year:'end_year' in p?p.end_year:null,description:'description' in p?p.description||'':'',location:'location' in p?p.location||'':'',employment_type:'employment_type' in p?p.employment_type||'':''})));setPositions(loadedPositions);savedPositions.current=JSON.stringify(loadedPositions);if(edit)setEditing(leadership?'Leadership':'Experience');}
