...
import { rankAutocomplete } from "@/lib/autocomplete";
...
export default function StudiesFields({ school, program, year, onProgram, onYear }: {
  ...
  const choices = rankAutocomplete(PROGRAMS[school] || [], program, name => name, name => [shortProgramName(name, school)]);
  ...
  return <div className={styles.studyChips} data-active={active}>
    <div className={`${styles.inputChip} ${styles.programChip}`} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setOpen(false); }}>
      ...
      {open && <ul id="program-options" role="listbox" aria-label="Programs" className={styles.programOptions}>{choices.map((name, index) => <li id={`program-option-${index}`} key={name} role="option" aria-selected={index === highlight} onMouseDown={event => event.preventDefault()} onClick={() => choose(name)} onMouseEnter={() => setHighlight(index)}><span>{name}</span><small>{shortProgramName(name, school) !== name ? shortProgramName(name, school) : ""}</small></li>)}{!choices.length && <li role="presentation">No matching programs</li>}</ul>}
    </div>
